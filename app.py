...
st.write("InstagramのDM、メール、または以下のフォームよりご連絡ください。")
...
with st.form("contact_form"):
    ...
    booking_date = st.date_input("ご希望日", min_value=datetime.date.today())
    ...
    if submitted:
        ...
        if booking_date in fully_booked_dates:
            st.error(f"申し訳ありません。{booking_date} は既に満席です。")
        elif not name or not email:
            st.warning("お名前とメールアドレスを入力してください。")
        else:
            # --- メール送信処理（FormSubmitを利用） ---
            # 送信先のアドレス
            url = "rieanpanman@gmail.com"
            ...
            data = {
                ...
                "メールアドレス": email,
                "希望メニュー": menu,
                "希望日時": f"{booking_date} {booking_time}",
                "メッセージ": message,
                "_captcha": "false", # スパム認証をオフにする設定
                "_subject": "【予約リクエスト】ホームページより問い合わせがありました" # メールの件名
            }
            ...
            if response.status_code == 200:
                ...
                担当者（rieanpanman@gmail.com）にメール通知を送りました。
                確認次第、{email} 宛にご連絡いたします。
                """)
            else:
                st.error("送信に失敗しました。お手数ですがInstagramのDMからご連絡ください。")